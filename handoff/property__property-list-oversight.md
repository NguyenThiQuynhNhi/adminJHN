...
2. **Buy/Rent** — dropdown: "All (Buy/Rent)" (default), "For Sale", "For Rent", "Sale + Rent". Applies immediately on change.
...
3. **Transaction** — a badge: "For Sale" (success variant), "For Rent" (warning variant), "Sale + Rent" (members/accent variant), or "—" (neutral variant) when none. Group 3 (Land) with no transaction value shows "For Sale".
...
5. **Price** — formatted price. For "For Rent" it shows "¥…/mo"; for "Sale + Rent" it shows "¥buy · ¥rent/mo"; otherwise the buy price.
...
- Transaction Type ★ (dropdown: "For Sale", "For Rent", "Sale + Rent"; full width)
- (Selling block — shows only for "For Sale" / "Sale + Rent"): header "— Selling Price"; Price (Buy) ★ (yen); Tax (dropdown: "Tax included", "No Tax included"); Expected Annual Income (fully occupied) (yen); Expected Monthly Income (fully occupied) (yen); Estimated Gross Yield (%) (percent); Price per m² (computed); Monthly Management Fee (¥); Monthly Repair Reserve Fund (¥); Lump-sum Repair Reserve (¥) (helper "Paid at handover"); Management Association Fee (¥); Ground Rent (¥).
- **Gross Yield (%)** — shows only when Current Status is "Tenanted (Owner Change)".
- (Rental block — shows only for "For Rent" / "Sale + Rent"): header "— Rental Details"; Rental Price Monthly (¥) ★; Tax (dropdown taxes); Unit Price per m² (computed); Security Deposit (months) (number); Security Deposit Amount (¥); Key Money (yen); Key Money Tax (dropdown taxes); Management & Common Service Fees (¥); Renewal Fee (yen); Renewal Fee Tax (dropdown taxes); Rental Contract Type (dropdown: "Ordinary Lease", "Fixed Term Lease"); Contract Period (text; placeholder "Duration (YY/MM) or Ends date").

**Transaction Type, Price & Occupancy (Group 2):**

- Transaction Type ★ (full width; same three options)
- (Selling block — "For Sale" / "Sale + Rent"): header "— Selling Price"; Price (Buy) ★; Tax; Price per m² (computed).
- (Rental block — shows only when transaction is "For Rent"/"Sale + Rent" AND property type is "House"): header "— Rental Details (House only)"; Rental Price Monthly (¥); Tax.
...
**Change History drawer** (opens from the "View change history" toolbar link in the detail view). A right-hand slide-out drawer (~520px wide) over a dark backdrop. Header: "Change History — {listing ID}" with an × close button; subtitle "Full edit log for this listing". A drawer filter bar with two dropdowns: **Actor** ("All actors" / "Agent only" / "Admin only") and **Field** ("All fields" + one option per distinct field present in this listing's history). Body lists the listing's `changeHistory[]` entries **newest-first**, each row showing the timestamp formatted "YYYY-MM-DD HH:MM" · actor name + a role badge (Agent = success/accent variant, Admin = members variant) · the field label · and the change "Old: {oldVal} → New: {newVal}". Long-text fields (Description, Notes / soldNotes) display "(Text modified)" instead of the raw old→new values. The drawer paginates at **50 entries per page** (Prev/Next inside the drawer footer, with an "{start}–{end} of {total}" counter). Empty state (no entries, or none matching the filters): "No changes recorded yet for this listing." Footer also has an **"Export change log to CSV"** button → toast "Exported change log to CSV." (no file generated). Closes via × icon, backdrop click, or ESC. Changing either filter dropdown re-renders and resets to drawer page 1. (The `changeHistory[]` data is seeded per listing — see Demo seed.)
...
2. **Buy/Rent** — Buy/Rent segmented toggle (default Buy → "For Sale"; Rent → "For Rent").
...
- The Selling Price block (and its Price/Tax/income/yield fields) shows only when Transaction Type is "For Sale" or "Sale + Rent".
- The Rental Details block shows only when Transaction Type is "For Rent" or "Sale + Rent". In Group 2 the rental block additionally requires the property type to be "House".
...
**View change history:** in the detail view click "View change history" → the right-hand drawer opens listing the property's edit log newest-first. Filter by Actor (Agent/Admin) or Field, page through 50-at-a-time, click "Export change log to CSV" for a toast, and close via × / backdrop / ESC.
...
**Change-history audit trail:** the `changeHistory[]` array is populated per listing and surfaced in the **Change History drawer** (actor/role/field/old→new/timestamp rows, newest-first, Actor/Field filters, 50/page, CSV-export toast). Long-text fields (Description, Notes/soldNotes) render as "(Text modified)" rather than dumping the full before/after text. Sale-related changes (mark-as-sold) already append a history-style record conceptually and should be logged to `changeHistory` when real edits are wired to the audit trail.
