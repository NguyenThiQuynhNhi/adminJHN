...
3. **Transaction** — a badge driven by the project's **Transaction Type** field (Basic Information): Buy → success (green), Rent → warning (amber) → accent/members variant. (No longer hardcoded "Buy"; missing values fall back to "Buy".)
...
- **Buy/Rent dropdown** — single-select, between Search and Property Type. Options: "All (Buy/Rent)" (no filter), "Buy", "Rent". Filters on the project's **Transaction Type**. Two-way synced with the Advanced modal's Buy/Rent toggle. Applies immediately on change.
...
**Change History drawer (PART A).** A right slide-out drawer (~520px, dark backdrop) opened by the detail toolbar's "View change history" link. Header "Change History — {project ID}" + an "×" close, subtitle "Full edit log for this project". A drawer filter bar with two selects: **Actor** (All actors / **Admin** / **Agency**) and **Field** (All fields / one option per distinct field present in this project's log, plus a **"Reports"** option when the project has report events). Body lists the project's `changeHistory` merged with its report history, newest-first:
- **Edit rows** show the timestamp ("YYYY-MM-DD HH:MM") · an **actor badge of either "Admin" (members variant) or "Agency" (success/accent variant)** — individual agent names are no longer shown; every non-admin edit collapses to **Agency** · the field label · and the change. **Short values** render inline as "Old → New"; **long values** (Description, Notes, or any value over ~60 chars) render in an **expandable "View change" panel** revealing the full **Before → After** (replacing the old "(Text modified)" placeholder).
...
- **Actor filter** "Admin" shows admin edits; "Agency" shows all non-admin edits; reports are hidden when a specific actor is chosen. **Field filter** "Reports" isolates the report events.

Paginated 50 per page (prev/next when needed). Empty state: "No changes recorded yet for this project." Footer has a "Export change log to CSV" button (fires a toast only). Closes three ways: the "×", a backdrop click, or ESC.
...
**Change history (drawer now built — PART A).** Every project carries a `changeHistory` array of audit entries `{ts, actorId, actorName, role, field, oldVal, newVal}` (seed ships 3–5 per project), and a subset also carries a `reports[]` array of report events (`{ts, reason}`). The detail toolbar's "View change history" link opens a right slide-out drawer titled "Change History — {project ID}" that lists entries (edits + reports) newest-first, filterable by Actor (All / **Admin** / **Agency**) and Field (distinct fields, plus a "Reports" option), paginated 50/page, with an "Export change log to CSV" (toast only) and an empty state. The actor is shown only as **Admin** or **Agency** (Agency = all non-admin roles; no individual agent names). Report events appear as "Reported — {reason}" rows (reasons: Inappropriate content / Suspected duplicate / Misleading price / Wrong photos). Long-value changes (Description, Notes, or any value over ~60 chars) render in an **expandable "View change" panel** showing the full Before → After (replacing the old "(Text modified)" placeholder); short values show inline old→new. BA note: future writes should append an entry whenever a tracked field (publish status, sales status, transaction type, brokerage type, per-unit sale status, completion date, etc.) changes, recording who (actor + role) and the before/after values.

**Brokerage Type vs Transaction Type (schema fix).** The Project Information field formerly labelled "Transaction Type" (options Owner / Agent / Exclusive Brokerage / Exclusive Agency / General Brokerage — describing the brokerage relationship) is now labelled **"Brokerage Type"** (schema key `txTypeProject` is unchanged; only the displayed label changed). A genuinely new field **"Transaction Type"** (schema key `transactionType`) was added to Basic Information with options **Buy** (default) / **Rent** / ****, required. It is the deal type and now drives both the list "Transaction" column badge (Buy → success, Rent → warning → accent) and the new hero Transaction Type badge, and is filterable via the compact bar's Buy/Rent dropdown and the Advanced modal's Buy/Rent toggle. The demo seed mixes the values roughly 80% Buy / 10% Rent / 10%  (each value present on ≥5–6 projects).
