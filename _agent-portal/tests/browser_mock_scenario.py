...
try:
 ...
 value('previewRole','Sales Agent',True);ready()
 check('Dashboard View role receives agency-level metric data',c.js(f"AgencyDashboardModel.evaluate(64,AgencyDashboardData.read('properties'),AgencyDashboardData.context()).value==={agency_value}"))
 check('No Dashboard Own or Agency scope wording',c.js("!document.body.innerText.includes('My activity')&&!document.body.innerText.includes('Agency scope')&&!document.body.innerText.includes('Own only')"))
 value('previewRole','Reception',True);c.wait("document.body.innerText.includes('Dashboard access unavailable')")
 check('Role without Dashboard View cannot render Dashboard',c.js("!document.querySelector('main')&&document.body.innerText.includes('Dashboard View permission')"))
 value('previewRole','Agency Admin',True);ready()
