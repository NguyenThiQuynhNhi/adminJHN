...
backup=c.js("Object.fromEntries(['yuushi.agency.dashboard.mock.v2','yuushi.ads.performanceSuppressed'].map(k=>[k,localStorage.getItem(k)]))")
try:
 c.js("localStorage.removeItem('yuushi.agency.dashboard.mock.v2');localStorage.setItem('yuushi.ads.performanceSuppressed','false')")
 c.call('Page.reload');ready()
 check('Overview defaults render',c.js("document.querySelectorAll('.widget').length===7"))
 agency_value=c.js("AgencyDashboardModel.evaluate(64,AgencyDashboardData.read('properties'),AgencyDashboardData.context()).value")
 check('Dashboard View context receives agency-level metric data',c.js(f"AgencyDashboardModel.evaluate(64,AgencyDashboardData.read('properties'),AgencyDashboardData.context()).value==={agency_value}"))
 check('No preview or permission-debug controls',c.js("!document.querySelector('#previewRole,#previewPremium,#previewSuppression,.preview-controls,.context-strip')&&!document.body.innerText.includes('CRM Premium')&&!document.body.innerText.includes('Dashboard View')&&!document.body.innerText.includes('Ad details hidden')&&!document.body.innerText.includes('Ad details enabled')"))
 c.js("window.qaContext=AgencyDashboardData.context;const base=qaContext();AgencyDashboardData.context=()=>({...base,role:'Reception',dashboardPermission:{view:false,export:false}});AgencyDashboard.refreshAccess()")
 c.wait("document.body.innerText.includes('Dashboard access unavailable')")
 check('Role without Dashboard View receives no Dashboard content or preview override',c.js("!document.querySelector('main')&&!document.querySelector('#previewRole')"))
 c.js("AgencyDashboardData.context=qaContext;AgencyDashboard.refreshAccess()")
 ready()
 ...
 c.js("localStorage.setItem('yuushi.ads.performanceSuppressed','true');dispatchEvent(new StorageEvent('storage',{key:'yuushi.ads.performanceSuppressed'}))");ready();click('add-widget')
 ...
 c.js("localStorage.setItem('yuushi.ads.performanceSuppressed','false');dispatchEvent(new StorageEvent('storage',{key:'yuushi.ads.performanceSuppressed'}))");ready();click('save')
 c.js("document.querySelector('[data-action=switch][data-id=subscriptions]').click()");ready()
