...
class DashboardTests(unittest.TestCase):
 def setUp(self):
  ...
  self.c.eval("var M=AgencyDashboardModel;var C={mode:'preview',agencyId:'AG-1',staffId:'STAFF-1',dashboardPermission:{view:true,export:true},suppression:false};var now=new Date('2026-09-08T04:00:00Z');function run(n,rows,w={},g={},ctx=C){return M.evaluate(n,{rows,mode:'preview',complete:true},ctx,w,g,now)}")
...
class RenderingAndAccessTests(unittest.TestCase):
 ...
 def test_permission_plan_and_suppression_matrix(self):
  result=self.js("""(()=>{const bad=[];let checks=0,base=D.context();for(const view of [true,false])for(const suppression of [true,false]){const ctx={...base,dashboardPermission:{...base.dashboardPermission,view},suppression};for(const m of M.metrics){checks++;const r=run(m.no,{},ctx),gate=M.gate(m,ctx);if(gate?r.state!==gate.state:!['ready','empty'].includes(r.state))bad.push([view,m.no,r.state]);}}return {checks,bad}})()""")
  self.assertEqual(result['bad'],[]);print('\nCurrent scope permission matrix:',result)
 def test_binary_dashboard_access_and_agency_level_rows(self):
  self.assertTrue(self.js("(()=>{const admin=D.context(),sales={...admin,role:'Sales Agent',staffId:S.staff.find(s=>s.role==='Sales Agent').staffId},denied={...admin,role:'Reception',staffId:S.staff.find(s=>s.role==='Reception').staffId,dashboardPermission:{view:false,export:false}},adminValue=run(64,{},admin).value,salesValue=run(64,{},sales).value;return M.canViewDashboard(admin)&&M.canViewDashboard(sales)&&adminValue===salesValue&&!M.canViewDashboard(denied)&&run(64,{},denied).state==='locked'})()"))
  self.assertTrue(self.js("(()=>{const ctx={...D.context(),role:'Sales Agent',staffId:S.staff.find(s=>s.role==='Sales Agent').staffId},rows=M.scoped(S.transactions,M.sourceMap.transactions,ctx);return rows.some(r=>r.staffId!==ctx.staffId)&&M.metrics.find(m=>m.no===324).dimensions.includes('staffId')})()"))
 def test_destination_permissions_remain_separate(self):
  self.assertTrue(self.js("(()=>{const base=D.context(),ctx={...base,role:'Sales Agent',permissions:{...base.permissions,projects:{scope:'none',actions:{view:false}}}};return M.canViewDashboard(ctx)&&!M.canAccessDestination(M.sourceMap.projects,ctx,'view')&&M.evaluate(118,D.read('projects'),ctx).state!=='locked'})()"))
 def test_no_preview_or_crm_premium_state(self):
  self.assertFalse(self.js("'features' in D.context()"))
  self.assertTrue(self.js("M.metrics.concat(M.systemWidgets).every(m=>!('premium' in m))"))
  self.assertTrue(self.js("(()=>{const denied={...D.context(),dashboardPermission:{view:false,export:false}},snapshot=D.read('properties',denied);return snapshot.state==='unavailable'&&snapshot.rows.length===0&&!snapshot.complete})()"))
