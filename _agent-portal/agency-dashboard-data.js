...
(function(root){
  ...
  function context(){
    ...
    return {mode:'preview',staffId:user.staffId,agencyId:user.agencyId,name:user.name,agentRating:permissions.profile?.actions.view?store.profile[0].rating:null,agencyName:store.profile[0].name,role,dashboardPermission,permissions,policyMap:Object.fromEntries(M.sources.map(s=>[s.id,s.id])),features:{crmPremium:p.crmPremium!==false},suppression};
  }
  function setPreview(p){const next={...preferences(),...p};localStorage.setItem(key,JSON.stringify(next));if('suppression' in p)localStorage.setItem('yuushi.ads.performanceSuppressed',JSON.stringify(p.suppression));try{root.parent?.syncDashboardNavigation?.();}catch{}}
