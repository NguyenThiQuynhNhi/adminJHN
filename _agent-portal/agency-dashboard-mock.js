...
(function(root){
  ...
  function create(now=new Date()){
    ...
    const cities=['Shinjuku','Shibuya','Minato','Yokohama','Osaka','Kyoto'];
    const prefectures=['Tokyo','Tokyo','Tokyo','Kanagawa','Osaka','Kyoto'];
    ...
    const properties=make(40,i=>({...base('PROP',i),propertyId:id('PROP',i),property:choose(cities,i)+' Residence '+(i+1),title:choose(cities,i)+' Residence '+(i+1),contactId:contacts[i].id,status:choose(['Published','Published','Pending Review','Published','Suspended','Rejected','Published','Draft'],i),draft:i%8===7,propertyType:choose(types,i),prefecture:choose(prefectures,i),city:choose(cities,i),intent:i%5===0?'Rent':'Buy',value:i%5===0?180000+i*15000:35000000+i*2500000,priceBasis:i%5===0?'JPY/month':'JPY sale',datePublished:date(i<32?-300+i*5:-(39-i)*3),dateUpdated:date(-i*3)}));
    const projects=make(15,i=>({...base('PROJECT',i),projectId:id('PROJECT',i),property:'New Development '+(i+1),title:'New Development '+(i+1),status:choose(['Published','Pending Review','Rejected','Suspended'],i),developer:choose(['Mori Development','Sakura Homes','Pacific Estates'],i),prefecture:choose(prefectures,i),city:choose(cities,i),datePublished:date(-i*19-30),completionDate:date(i*12-30),inventory:make(6,j=>({id:id('UNIT-'+(i+1),j),projectId:id('PROJECT',i),quantity:i%2===0?j+2:1,status:(i+j)%3===0?'Sold Out':'On Sale'}))}));
