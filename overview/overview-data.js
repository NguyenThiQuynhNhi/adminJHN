...
(function(root) {
  ...
  // Agency business accounts; legacy agents/agentId remain compatibility aliases.
  const agencies=agents;
  const subscriptions=agencies.filter(a=>a.plan!=='Free').map((a,i)=>({
    id:'SUB-'+a.id,agencyId:a.id,status:a.status==='Pending'?'Pending':i%7===0?'Cancelled':'Active',
    recurring:true,billingCycle:i%2?'Annual':'Monthly',fee:prices[a.plan]*(i%2?12:1),start:a.registeredAt,end:null
  }));
  // Separate Admin support queue, never used as Property Inquiry data.
  const adminChats=Array.from({length:20},(_,i)=>({id:'ADMIN-CHAT-'+i,recipient:'YUUSHI Admin',status:i<14?'unresponded':'responded'}));
  // Sessions are attributed to Client or Agency business accounts, never staff identities.
  const accountSessions=accounts.flatMap(a=>a.logins.map(date=>({accountId:a.id,date,seconds:90+hash(a.id+date)%390})));
  ...
  // Daily Property Inquiry aggregates: Client → Agency via Chat with Agency on Property Detail.
  // Deals represent existing Agency-recorded Transactions. No repeat-inquiry deduplication is assumed.
  const facts=[];
  ...
  [properties,facts,chats].forEach(rows=>rows.forEach(r=>r.agencyId=r.agentId));
  ...
  const data={today,now,accounts,agencies,agents,subscriptions,adminChats,accountSessions,guests,properties,facts,chats,fraudFlags,paymentErrors,logs,places,groups,structures,countries,plans,prices,adTypes,options};
