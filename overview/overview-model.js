...
(function(root) {
  ...
  function metric(spec,f) {
    ...
    if(id==='r29')return result(buckets(U.addDays(D.today,-29),D.today,'Daily').map(b=>({label:b.label,registrations:platformAccounts(D.accounts,b.end).filter(a=>a.registeredAt===b.end).length,customers:activity(D.accounts.filter(a=>a.type==='Client'),b.end,1),agents:activity(D.agents,b.end,1)})),['registrations','customers','agents'],'line');
    ...
    if(id==='r32')return {...timeline(['inquiries','deals'],'bar'),note:'Property Inquiries from Clients to Agencies and Agency-recorded deals.',columns:[{key:'inquiries',label:'Property Inquiries',unit:''},{key:'deals',label:'Agency deals',unit:''}]};
    ...
    if(id==='r46')return result(groupBy(users,f.breakdown==='nationality'?'nationality':'country',r=>({count:r.length})),['count'],'donut',f.breakdown==='nationality'?'Client nationality distribution.':'Client country of residence distribution.');
    ...
    if(id==='r75'){
      ...
      return result(D.adTypes.map(type=>({label:type,count:pool.filter(a=>a.adProducts.includes(type)).length,rate:ratio(pool.filter(a=>a.adProducts.includes(type)).length,pool.filter(a=>a.adProducts.length).length)})),['count','rate'],'bar','Share among ad-using Agencies in the selected account area and property segment. Agencies can use multiple ad types; shares need not total 100%.');
    }
    ...
    if(id==='r83'){
      ...
      return f.breakdown==='subtype'?result(allowed.map(type=>({label:type,count:pool.filter(a=>a.appraisalTypes.includes(type)).length})),['count'],'bar','Registered appraisal Agencies by supported property type. An Agency can support multiple types.'):result(groupBy(pool,'prefecture',items=>({count:items.length})),['count'],'donut','Registered appraisal Agencies by area, filtered by supported property types.');
    }
